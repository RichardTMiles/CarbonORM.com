...
import {C6, iRestfulObjectArrayTypes, iWp_Postmeta, wp_postmeta } from "api/rest/C6";
import Wp_Postmeta from "./Wp_Postmeta";
...
xdescribe('REST Wp_Postmeta api', () => {
    ...
    test('GET POST PUT DELETE', async () => {
        ...
        await act(async () => {
            ...
            let selectAllResponse = await Wp_Postmeta.Get({})
            ...
            const postResponse = await Wp_Postmeta.Post(testData);

            console.log('postResponse', postResponse?.data)

            expect(postResponse?.data?.created).not.toBeUndefined();

            const primaryKey = wp_postmeta.PRIMARY_SHORT[0];

            const postID = postResponse?.data?.created

            const singleRowSelect = await Wp_Postmeta.Get({
                [C6.WHERE]: {
                    [wp_postmeta[primaryKey.toUpperCase()]]: postID,
                }
            })
            ...
            const multipleRowSelect = await Wp_Postmeta.Get({
                [C6.WHERE]: {
                    [wp_postmeta[primaryKey.toUpperCase()]]: [C6.IN, [0, postID]],
                }
            })
            ...
            expect(CarbonReact.getState<iRestfulObjectArrayTypes>().wp_postmeta).not.toBeUndefined();

            const updateResponse = await Wp_Postmeta.Put(testData)

            expect(updateResponse?.data?.updated).not.toBeUndefined();

            const deleteResponse = await Wp_Postmeta.Delete({
                [primaryKey]: postID
            })
